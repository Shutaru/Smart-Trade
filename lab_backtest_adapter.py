...
class StrategyLabBacktestEngine:
    ...
    Evaluates visual strategy configurations against historical data
    """
...
# Get right-hand side value (RHS)
 ...
 return lhs >= rhs
    ...
    return abs(lhs - rhs) < 1e-9
     ...
     # TODO: Implement BETWEEN logic (needs two RHS values)
 return False
 ...
 risk = self.config.risk
    ...
    if risk.position_sizing == "fixed_usd":
     ...
     sl_mult = 2.0  # TODO: Make configurable
        tp_mult = 3.0  # TODO: Make configurable (Risk:Reward ratio)
 ...
 R = sl - price
        ...
        if isinstance(leverage, tuple):
         leverage = leverage[0]  # Use minimum if range
   ...
   'sortino': em.get('sharpe_ann', 0.0) * 1.2,  # Approximation
    ...
    def _save_artifacts(self, broker: PaperFuturesBroker, metrics: Dict[str, Any]):
     """
        ...
        Files generated:
        - trades.csv (already created by broker)
        - equity_curve.csv (already created by broker)
 - equity.csv (formatted for frontend)
        - metrics.json
        """
...
def run_strategy_lab_backtest(config: StrategyConfig, artifact_dir: str) -> Dict[str, Any]:
  ...
  config: Strategy configuration from Strategy Lab
        artifact_dir: Directory to save results (trades.csv, equity.csv, etc)
    
    Returns:
   Dictionary with backtest metrics (sharpe, total_profit, win_rate, etc)
    
    Raises:
        ValueError: If config is invalid or data is insufficient
        FileNotFoundError: If historical data not found (need to backfill first)
    """
