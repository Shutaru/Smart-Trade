...
export function StrategyBuilder({ value, onChange }: StrategyBuilderProps) {
    ...
    const updateCondition = (side: 'long' | 'short', type: 'entry_all' | 'entry_any', index: number, updates: Partial<Condition>) => {
        const newValue = { ...value };
        newValue[side][type][index] = { ...newValue[side][type][index], ...updates };
        onChange(newValue);
    };

    const removeCondition = (side: 'long' | 'short', type: 'entry_all' | 'entry_any', index: number) => {
        const newValue = { ...value };
                                ...
                                <TableRow key={index}>
                                    <TableCell>
                                        <Select
                                            ...
                                            onValueChange={(v) => {
                                                const indicator = indicatorsData?.indicators?.find((ind: any) => ind.id === v);
                                                const defaultParams = indicator ?
                                                    Object.entries(indicator.params).map(([name, config]: [string, any]) => ({
                                                        name,
                                                        value: config.default
                                                    })) : [];
                                                updateCondition(side, type, index, { indicator: v, params: defaultParams });
                                            }}
                                        >
                                    ...
                                    <TableCell>
                                        <Input
                                            type="number"
                                            value={condition.rhs || ''}
                                            onChange={(e) => updateCondition(side, type, index, { rhs: parseFloat(e.target.value) })}
                                            placeholder="Value"
                                            className="w-[120px]"
                                        />
                                    </TableCell>
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Define entry conditions for long and short positions</CardDescription>
            </CardHeader>
            <CardContent>
                <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'long' | 'short')}>
